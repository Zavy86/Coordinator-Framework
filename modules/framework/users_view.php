 ...
 */
 ...
 foreach($user_obj->getAssignedGroups() as $group_f){
  ...
  $groups_table->addRowField(api_link("?mod=".MODULE."&scr=groups_view&idGroup=".$group_obj->id,$group_obj->fullname." (".$group_f->level.")",$group_obj->getPath("string"),"hidden-link",true,null,null,null,"_blank"),"truncate-ellipsis");
 ...
 if(ACTION=="group_add"){
  ...
  $group_add_form=new cForm("?mod=".MODULE."&scr=submit&act=user_group_add&idUser=".$user_obj->id,"POST",null,"users_view-groups_modal");
  ...
  $group_add_form->addField("select","level",api_text("users_view-groups_modal-ff-level"),null,api_text("users_view-groups_modal-ff-level-placeholder"),null,null,null,"required");
  for($level=1;$level<=$GLOBALS['settings']->users_level_max;$level++){$group_add_form->addFieldOption($level,api_text("users_view-groups_modal-fo-level",$level));}
