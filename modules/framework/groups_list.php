...
foreach($groups_array as $group_fobj){
	...
	if($group_fobj->nesting>$nesting_previous){
		// check if previous value was the last nesting
		if($nesting_last){$nesting_string=$nesting_string."_";}
		else{$nesting_string=$nesting_string."I";}
		$nesting_previous=$group_fobj->nesting;
	}
	// check if nesting is decremented
	if($group_fobj->nesting<$nesting_previous){
		$nesting_string=substr($nesting_string,0,$group_fobj->nesting);
		$nesting_previous=$group_fobj->nesting;
	}
	// check if is the last nesting
	if($group_fobj->nesting_last){$item_nest_string=$nesting_string."L";}
	else{$item_nest_string=$nesting_string."T";}
	// replace chars
	$final_nest_string=str_replace(array("_","I","L","T"),array("&nbsp;&nbsp;&nbsp;","│&nbsp;&nbsp;","└─&nbsp;","├─&nbsp;"),$item_nest_string);
	// save if was the last nesting
	$nesting_last=$group_fobj->nesting_last;
	...
	$table->addRowField(api_tag("samp",$final_nest_string).$group_fobj->name,"nowrap");
